...
namespace Bots {
    void SpawnPlayerBot(AActor* OverrideSpawnLoc = nullptr, PlayerBots::EBotState StartingState = PlayerBots::EBotState::Warmup) {
        ...
        if (Floor && Roof && Stair && Wall && EditTool) {
            ...
            // Add building materials if globals allow building
            if (Globals::bBotBuildingEnabled) {
                static UFortResourceItemDefinition* WoodMaterial = StaticLoadObject<UFortResourceItemDefinition>("/Game/Items/ResourceItems/WoodItemData.WoodItemData");
                static UFortResourceItemDefinition* StoneMaterial = StaticLoadObject<UFortResourceItemDefinition>("/Game/Items/ResourceItems/StoneItemData.StoneItemData");
                static UFortResourceItemDefinition* MetalMaterial = StaticLoadObject<UFortResourceItemDefinition>("/Game/Items/ResourceItems/MetalItemData.MetalItemData");

                if (WoodMaterial) BotCustomizationData->StartupInventory->Items.Add(WoodMaterial);
                if (StoneMaterial) BotCustomizationData->StartupInventory->Items.Add(StoneMaterial);
                if (MetalMaterial) BotCustomizationData->StartupInventory->Items.Add(MetalMaterial);
            }
        }
        else
        {
        ...
        // ENHANCED: POI-based spawn selection
        AActor* BotSpawn = nullptr;
        
        if (Globals::bBotPOIBasedSpawning && OverrideSpawnLoc == nullptr) {
            const MapPOIs::FPOIInfo* POI = MapPOIs::GetWeightedRandomPOI();
            if (POI) {
                FVector SpawnLocation = MapPOIs::GetRandomLocationInPOI(POI);
                FRotator SpawnRotation = FRotator(0, UKismetMathLibrary::GetDefaultObj()->RandomFloatInRange(0.f, 360.f), 0);
                
                // Create a temporary actor for spawn location
                BotSpawn = SpawnActor<AActor>(SpawnLocation, SpawnRotation);
                Log("Spawning bot at POI: " + std::string(POI->Name.ToString().begin(), POI->Name.ToString().end()));
            }
        }
        
        // Fallback to player starts if POI spawning failed or is disabled
        if (!BotSpawn) {
            if (BotSpawnLocationIndex >= MaxSpawnLocations) {
                BotSpawnLocationIndex = 0;
            }
            BotSpawn = PlayerStarts[BotSpawnLocationIndex];
