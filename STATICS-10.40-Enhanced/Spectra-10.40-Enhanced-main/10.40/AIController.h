            ...
            for (PlayerBots::PlayerBot* bot : PlayerBots::PlayerBotArray)
            {
                if (bot->PC == Comp->BotController)
                {
                    ...
                    if (bot->bPotentiallyUnderAttack) {
                        ...
                        // ENHANCED: Try building when bumped during combat
                        if (Globals::bBotBuildingEnabled && UKismetMathLibrary::GetDefaultObj()->RandomBoolWithWeight(0.2f)) {
                            bot->CurrentBuildAction = PlayerBots::EBotBuildAction::BoxUp;
