...
namespace PlayerBots {
    ...
    public:
        // Building system
        EBotBuildAction CurrentBuildAction = EBotBuildAction::MAX;
        float TimeToNextBuildAttempt = 0.f;
    ...
    public:
        ...
        void BuildStructure(EBotBuildingType BuildingType, FVector Location, FRotator Rotation)
        {
            if (!Globals::bBotBuildingEnabled) {
                return;
            }
            
            UFortBuildingItemDefinition* BuildingDef = nullptr;
            ...
            if (CantBuild && CantBuild(UWorld::GetWorld(), BuildingDef, Location, Rotation, 1, nullptr, nullptr)) {
                return;
            }
            ...
            if (ReplaceBuildingActor) {
                ABuildingSMActor* Building = (ABuildingSMActor*)ReplaceBuildingActor(
                    nullptr, 
                ...
                if (Building) {
                    Building->K2_SetActorLocation(Location, false, &HitResult, true);
                    Building->K2_SetActorRotation(Rotation, true);
                    Building->SetReplicates(true);
                    Building->SetReplicateMovement(true);
                }
            }
        }
        
        void PerformBuildAction()
        {
            if (!Globals::bBotBuildingEnabled || bIsBuilding) {
                return;
            }
            
            float CurrentTime = UGameplayStatics::GetDefaultObj()->GetTimeSeconds(UWorld::GetWorld());
            
            if (CurrentTime < TimeToNextBuildAttempt) {
                return;
            }
            
            FVector BotLoc = Pawn->K2_GetActorLocation();
            FRotator BotRot = Pawn->K2_GetActorRotation();
            
            switch (CurrentBuildAction) {
                case EBotBuildAction::Defensive1x1:
                    // Build 1x1 with roof
                    if (BuildAttemptCount < 4) {
                        FVector WallLoc = BotLoc + Pawn->GetActorForwardVector() * 200.f;
                        BuildStructure(EBotBuildingType::Wall, WallLoc, BotRot);
                        
                        WallLoc = BotLoc - Pawn->GetActorForwardVector() * 200.f;
                        BuildStructure(EBotBuildingType::Wall, WallLoc, BotRot);
                        
                        WallLoc = BotLoc + Pawn->GetActorRightVector() * 200.f;
                        BuildStructure(EBotBuildingType::Wall, WallLoc, BotRot);
                        ...
                        // Build roof
                        FVector RoofLoc = BotLoc + FVector(0, 0, 400.f);
                        BuildStructure(EBotBuildingType::Cone, RoofLoc, BotRot);
                        
                        BuildAttemptCount++;
                    } else {
                        CurrentBuildAction = EBotBuildAction::MAX;
                        BuildAttemptCount = 0;
                    }
                    break;
                    
                case EBotBuildAction::RampRush:
                    if (NearestPlayerPawn && BuildAttemptCount < 3) {
                        FVector EnemyLoc = NearestPlayerPawn->K2_GetActorLocation();
                        FVector Direction = UKismetMathLibrary::GetDefaultObj()->Normal(EnemyLoc - BotLoc);
                        FVector RampLoc = BotLoc + Direction * 300.f;
                        RampLoc.Z += 100.f * BuildAttemptCount;
                        
                        FRotator RampRot = UKismetMathLibrary::GetDefaultObj()->FindLookAtRotation(BotLoc, EnemyLoc);
                        RampRot.Pitch = -35.0f; // Stair angle
                        
                        BuildStructure(EBotBuildingType::Stair, RampLoc, RampRot);
                        BuildAttemptCount++;
                    } else {
                        CurrentBuildAction = EBotBuildAction::MAX;
                        BuildAttemptCount = 0;
                ...
                case EBotBuildAction::BoxUp:
                    // Quick box around self
                    if (BuildAttemptCount < 3) {
                        FVector BoxLoc = BotLoc + Pawn->GetActorForwardVector() * 150.f;
                        BuildStructure(EBotBuildingType::Wall, BoxLoc, BotRot);
                        
                        BoxLoc = BotLoc + Pawn->GetActorRightVector() * 150.f;
                        BuildStructure(EBotBuildingType::Wall, BoxLoc, BotRot);
                        
                        // Floor above
                        FVector FloorLoc = BotLoc + FVector(0, 0, 300.f);
                        BuildStructure(EBotBuildingType::Floor, FloorLoc, BotRot);
                ...
                case EBotBuildAction::StairToZone:
                    // Build stairs toward safe zone
                    if (BuildAttemptCount < 5) {
                        auto GameState = (AFortGameStateAthena*)UWorld::GetWorld()->GameState;
                        if (GameState && GameState->SafeZoneIndicator) {
                            FVector ZoneLoc = GameState->SafeZoneIndicator->NextCenter;
                            FVector Direction = UKismetMathLibrary::GetDefaultObj()->Normal(ZoneLoc - BotLoc);
                            FVector StairLoc = BotLoc + Direction * 300.f;
                            StairLoc.Z += 100.f * BuildAttemptCount;
                            
                            FRotator StairRot = UKismetMathLibrary::GetDefaultObj()->FindLookAtRotation(BotLoc, ZoneLoc);
                            StairRot.Pitch = -35.0f;
                            
                            BuildStructure(EBotBuildingType::Stair, StairLoc, StairRot);
                            BuildAttemptCount++;
                        } else {
                            CurrentBuildAction = EBotBuildAction::MAX;
                            BuildAttemptCount = 0;
            ...
            TimeToNextBuildAttempt = CurrentTime + 0.5f; // Cooldown between builds
        }
        ...
        bool ShouldBuild()
        {
            ...
            // Build when under attack and low health
            if (bPotentiallyUnderAttack && (Pawn->GetHealth() < 50.f || Pawn->GetShield() < 30.f)) {
                CurrentBuildAction = EBotBuildAction::BoxUp;
                return true;
            }
            
            // Build when healing
            if (BotState == EBotState::Healing) {
                CurrentBuildAction = EBotBuildAction::Defensive1x1;
                return true;
            }
            
            // Build ramps toward safe zone when far
            auto GameState = (AFortGameStateAthena*)UWorld::GetWorld()->GameState;
            if (GameState && GameState->SafeZoneIndicator && BotState == EBotState::MovingToSafeZone) {
                float DistToZone = UKismetMathLibrary::GetDefaultObj()->Vector_Distance(Pawn->K2_GetActorLocation(), GameState->SafeZoneIndicator->NextCenter);
                if (DistToZone > 5000.f) {
                    CurrentBuildAction = EBotBuildAction::StairToZone;
                    return true;
                }
            }
        ...
        bool IsLocationOnRoof(FVector Location)
        {
            // Perform downward raycast to detect if we're above a roof
            FVector TraceStart = Location;
            TraceStart.Z += 500.f; // Start above the location
            FVector TraceEnd = Location;
            TraceEnd.Z -= 1000.f; // Trace down below
            
            FHitResult HitResult;
            if (bot->PC) {
                bool bHit = bot->PC->LineTraceSingle(
                    TraceStart, 
                    TraceEnd, 
                ...
                if (bHit && HitResult.Actor) {
                    // Check if we hit a building actor (roof)
                    if (HitResult.Actor->IsA(ABuildingSMActor::StaticClass())) {
                        return true;
                    }
                }
            }
        ...
        FVector FindRoofLandingLocation(FVector BaseLocation)
        {
            // Search for nearby buildings with roofs
            TArray<AActor*> BuildingActors;
            UGameplayStatics::GetDefaultObj()->GetAllActorsOfClass(UWorld::GetWorld(), ABuildingSMActor::StaticClass(), &BuildingActors);
            
            FVector BestRoofLocation = BaseLocation;
            float BestHeight = BaseLocation.Z;
            
            for (int i = 0; i < BuildingActors.Num(); i++) {
                ABuildingSMActor* Building = (ABuildingSMActor*)BuildingActors[i];
                if (!Building) continue;
                
                FVector BuildingLoc = Building->K2_GetActorLocation();
                float Dist = UKismetMathLibrary::GetDefaultObj()->Vector_Distance(BaseLocation, BuildingLoc);
                
                if (Dist < 1000.f) { // Within 1000 units
                    FVector RoofLocation = BuildingLoc;
                    RoofLocation.Z += 350.f; // Roof height
                    
                    if (RoofLocation.Z > BestHeight) {
                        BestHeight = RoofLocation.Z;
                        BestRoofLocation = RoofLocation;
                    }
                }
            }
            
            BuildingActors.Free();
            return BestRoofLocation;
        }
            ...
            if (TargetLootable) {
                ...
                if (!BotLoc.IsZero()) {
                    ...
                    if (TargetLootable) {
                        ...
                        // if the bot is not getting closer or stuck for more than 8 seconds then try gradual unstuck
                        if ((Elapsed > 8.f && Dist > bot->LastLootTargetDistance - 100.f)) {
                            // Try gradual unstuck before teleporting as last resort
                            if (Elapsed < 15.f) {
                                bot->Pawn->PawnStopFire(0);
                                ...
                                if (UKismetMathLibrary::GetDefaultObj()->RandomBoolWithWeight(0.5f)) {
                                    FVector RandomDir = UKismetMathLibrary::GetDefaultObj()->RandomUnitVector();
                                    bot->Pawn->AddMovementInput(RandomDir, 1.0f, true);
                                }
                                // Reset timer to give time for unstuck attempt
                                bot->LootTargetStartTime = UGameplayStatics::GetDefaultObj()->GetTimeSeconds(UWorld::GetWorld()) - 5.f;
                                return;
                            }
                            // As last resort after 15+ seconds, do line trace check before teleporting
                            if (Elapsed >= 15.f) {
                                bot->Pawn->PawnStopFire(0);
                                FHitResult HitResult;
                                FVector TraceStart = bot->ClosestLootableLoc;
                                TraceStart.Z += 1000.f;
                                FVector TraceEnd = bot->ClosestLootableLoc;
                                TraceEnd.Z -= 1000.f;
                                
                                // Do a vertical line trace to find ground
                                bot->PC->LineTraceSingle(TraceStart, TraceEnd, ETraceTypeQuery::TraceTypeQuery1, false, {}, EDrawDebugTrace::None, HitResult, true);
                                
                                FVector TeleportLoc = HitResult.bBlockingHit ? HitResult.Location + FVector(0, 0, 50.f) : bot->ClosestLootableLoc;
                                
                                if (!bot->Pawn->K2_TeleportTo(TeleportLoc, {})) {
                                    bot->Pawn->K2_SetActorLocation(TeleportLoc, false, &HitResult, true);
                                }
            ...
            if (bot->NearestPlayerPawn) {
                ...
                if (!Nearest.IsZero()) {
                    ...
                    if (bot->PC->LineOfSightTo(bot->NearestPlayerPawn, BotLoc, true)) {
                        // Apply aim accuracy based on bot skill level
                        float AccuracyOffset = 1.0f - Globals::BotAimAccuracy; // Convert accuracy to inaccuracy
                        
                        // Increase inaccuracy when stressed or moving
                        if (bot->bIsStressed) {
                            AccuracyOffset *= 1.5f;
                        }
                        
                        float RandomXmod = Math->RandomFloatInRange(-150, 150) * AccuracyOffset;
                        float RandomYmod = Math->RandomFloatInRange(-150, 150) * AccuracyOffset;
                        float RandomZmod = Math->RandomFloatInRange(-150, 150) * AccuracyOffset;
                        ...
                        // Occasionally aim directly at target (especially with high accuracy bots)
                        if (UKismetMathLibrary::GetDefaultObj()->RandomBoolWithWeight(Globals::BotAimAccuracy * 0.1f)) {
                        ...
                        // Implement consistent strafing pattern
                        float CurrentTime = UGameplayStatics::GetDefaultObj()->GetTimeSeconds(UWorld::GetWorld());
                        if (CurrentTime >= bot->StrafeEndTime) {
                            // Switch strafe direction periodically
                            bot->StrafeType = (bot->StrafeType == PlayerBots::EBotStrafeType::StrafeLeft) ? 
                                PlayerBots::EBotStrafeType::StrafeRight : PlayerBots::EBotStrafeType::StrafeLeft;
                            bot->StrafeEndTime = CurrentTime + Math->RandomFloatInRange(1.5f, 3.0f);
                            bot->ForceStrafe(true);
                        }
            ...
            if (GameState->GamePhase == EAthenaGamePhase::Warmup) {
                ...
                if (GameState && GameState->SafeZoneIndicator)
                {
                    ...
                    if (Elapsed > 10.f && DistToZone > Bot->LastZoneTargetDistance - 100.f)
                    {
                        ...
                        // Try building to get unstuck instead of launching into air
                        if (Globals::bBotBuildingEnabled && Bot->ShouldBuild()) {
                            Bot->PerformBuildAction();
                        } else {
                            // Last resort: jump or launch
